...
fun spendingsToCSV(
    ...
    for (i in info.reversed()) {
        val s = extractSpendingFromInfo(i)
        ...
        val line =
            ...
            },${s.currency},${i.categoryName},${i.subcategoryName ?: "-"},${s.date},${budgetDefault},${rsd},${eur},${usd}"
        data.add(line)
    }
    //total amount spent
    val budgetCurrencyTotal =
        String.format("%.2f", info.sumOf { convert(extractSpendingFromInfo(it), budget.defaultCurrency).price })
    val rsdTotal = String.format("%.2f", info.sumOf { convert(extractSpendingFromInfo(it), Currency.RSD).price })
    val eurTotal = String.format("%.2f", info.sumOf { convert(extractSpendingFromInfo(it), Currency.EUR).price })
    val usdTotal = String.format("%.2f", info.sumOf { convert(extractSpendingFromInfo(it), Currency.USD).price })
    ...
    SparicaDatabase.categories.entries.forEach { (category, subcategories) ->
        ...
        subcategories.forEach { subcategory ->
            val spentInSubcategory = info
                .filter { it.subcategoryName.equals(subcategory) && it.categoryName.equals(category) }
                .sumOf { convert(extractSpendingFromInfo(it), budget.defaultCurrency).price }
            val temp = Spending(
                description = "",
        ...
        if (subcategories.isEmpty()) {
            ...
            categoryDefault = info
                .filter { it.categoryName.equals(category) }
                .sumOf { convert(extractSpendingFromInfo(it), budget.defaultCurrency).price }
            val temp = Spending(
